...
class OfferRow:
    ...
    def total(self) -> float:
        return self.price * self.inp.quantity

    # data voor de HTML‑template
    def html(self) -> Dict[str, str]:
        detail = (
            f"{self.inp.colours}-kleur tampondruk – ontwerpcontrole"
...
# De oude fout werd veroorzaakt door Jinja2 die verward raakte met de CSS‑accolades.
# Door de CSS in een RAW‑blok te zetten met nette nieuwe regels is dat opgelost.
...
<head>
  ...
  <style>
    ...
    th,td{ padding:6px; border-bottom:1px solid #fff }
    th{ text-align:left }
...
<body>
  <div class="page">
    <div class="header">
      ...
      <p>
        Offertenr&nbsp;{{ client.offer_number }}<br>
    ...
    <div class="section">
      ...
      <table class="prod">
        ...
        {% endfor %}
        {% if special %}
        <tr><td>1</td><td>Extra</td><td>Special</td><td>Afwijkende kleur</td><td style="text-align:right">€ 480,00</td><td style="text-align:right">€ 480,00</td></tr>
        {% endif %}
        {% if ship %}
        <tr><td>1</td><td>Verzendkosten</td><td>-</td><td>Extra zending</td><td style="text-align:right">€ 150,00</td><td style="text-align:right">€ 150,00</td></tr>
        {% endif %}
      </table>
    </div>
...
ARTISAN = [
    ...
    (1000, (0.50, 0.57, 0.64)),
    (5000, (0.45, 0.52, 0.59)),
    (10000, (0.40, 0.47, 0.54)),
    (20000, (0.35, 0.42, 0.49)),
...
def _spuitgiet(q: int, t: str, special: bool) -> float:
    ...
    if special:
        base += EXTRA_KOSTEN["special"] / q
    return base


def _artisan(q: int, c: int) -> float:
    """Tampondruk‑kosten uit Artisan‑tabel."""
    for upper, prices in ARTISAN:
        if q <= upper:
            return prices[c - 1]
    raise ValueError("Aantal te hoog voor Artisan‑tabel")
...
def gui():
    ...
    st.set_page_config(page_title="Calix offerte generator", page_icon="📄", layout="wide")
        ...
        with st.expander(f"Variant {i + 1}", expanded=True):
            ...
            offers.append(
                dict(
                    ...
                    colours=int(cols),
                    band_color=band,
                    extra_increase_pct=up,
                    discount_pct=disc,
                )
            )

    if st.button("Genereer offerte"):
        # veiligheid: basisdata vereist
        if not (name and address and nr):
            st.warning("Vul eerst klantgegevens in.")
            st.stop()
        ...
        try:
            ...
            st.download_button(
                "Download PDF", pdf_bytes, file_name="offerte.pdf", mime="application/pdf"
            )
        except Exception as e:
...
if __name__ == "__main__":
    # Wordt het script door Streamlit aangeroepen?
    if "streamlit" in pathlib.Path(sys.argv[0]).name.lower():
        gui()
    # Geen argumenten → help‑tekst
    elif len(sys.argv) == 1:
        print(
            "Gebruik CLI:   python calix_quote.py offer_input.json --output-dir ./out\n"
