...
def html_to_pdf_bytes(html: str) -> bytes | None:
    ...
    return None            # geen backend beschikbaar
...
KLEURBANDJE_LST = ["Standaard", "Special", "Zwart", "Off White", "Blauw", "Rood"]
...
with colB:
    ...
    kleur_bandje   = st.selectbox("Kleur bandje", KLEURBANDJE_LST, index=2)  # standaard Zwart
...
if opties_aantal > 1:
    ...
    for i in range(2, opties_aantal + 1):
        ...
        with c4:
            kband = st.selectbox("Kleur bandje", KLEURBANDJE_LST,
                                  index=0, key=f"opt_band_{i}")
        kort = st.number_input("Korting (%)", 0.0, 100.0, 0.0, key=f"opt_kort_{i}")

        extra_opties.append(dict(aantal=a, type=t, kleuren=kc,
                                 band=kband, korting=kort))
...
if aantal > 10000:
    append_row(1, "Verzendkosten", "–", 150,
...
if pdf_data:
    ...
    st.info("PDF-backend niet beschikbaar op dit platform. Download de HTML en print die vervolgens als PDF.")
